...
class QuizCVCell: UICollectionViewCell {
    ...
    var question: QuestionFetched? {
        didSet {
            ...
            btn3.setTitle(unwrappedQue.choices[2], for: .normal)
            btn4.setTitle(unwrappedQue.choices[3], for: .normal)
            
            if unwrappedQue.isAnswered {
                btnsArray[unwrappedQue.correctAnswer].backgroundColor=UIColor.green
                if unwrappedQue.wrongAnswer >= 0 {
                    btnsArray[unwrappedQue.wrongAnswer].backgroundColor=UIColor.red
                }
            }
        }
    }
